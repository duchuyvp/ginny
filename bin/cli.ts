...
Environment variables:
  GINNY_PORT                     Port to listen on (default: 3456)
  GINNY_HOST                     Host to bind to (default: 127.0.0.1)
  GINNY_PASSTHROUGH              Enable passthrough mode (tools forwarded to client)
  GINNY_IDLE_TIMEOUT_SECONDS     Idle timeout in seconds (default: 120)

See https://github.com/rynfar/ginny for full documentation.`)
  process.exit(0)
}
...
if (args[0] === "setup") {
  ...
  if (result.alreadyConfigured) {
    console.log(`\x1b[32m✓ Ginny plugin already configured\x1b[0m`)
    ...
    console.log(`\x1b[32m✓ Ginny plugin configured\x1b[0m`)
...
const port = parseInt(process.env.GINNY_PORT ?? process.env.CLAUDE_PROXY_PORT ?? "3456", 10)
const host = process.env.GINNY_HOST ?? process.env.CLAUDE_PROXY_HOST ?? "127.0.0.1"
const idleTimeoutSeconds = parseInt(process.env.GINNY_IDLE_TIMEOUT_SECONDS ?? process.env.CLAUDE_PROXY_IDLE_TIMEOUT_SECONDS ?? "120", 10)

// Load profile configuration:
//   1. GINNY_PROFILES env var (JSON array) — takes precedence
//   2. ~/.config/ginny/profiles.json — written by `ginny profile add`
// fs/path/os imports removed — profile discovery now handled by the server

// Profile config: only set from GINNY_PROFILES env var.
// When undefined, the server auto-discovers from ~/.config/ginny/profiles.json
// on each request (so `ginny profile add` works without restart).
import type { ProfileConfig } from "../src/proxy/profiles"
let profiles: ProfileConfig[] | undefined
let defaultProfile: string | undefined
try {
  const raw = process.env.GINNY_PROFILES
  if (raw) {
    profiles = JSON.parse(raw)
    defaultProfile = process.env.GINNY_DEFAULT_PROFILE || undefined
  }
  // No else — let the server auto-discover from disk
} catch (e) {
  console.error(`[ginny] Failed to parse GINNY_PROFILES: ${e instanceof Error ? e.message : e}`)
}
...
export async function runCli(
  ...
  // Plugin check — warn if OpenCode config exists but ginny plugin is missing
  try {
    ...
    if (existsSync(configPath) && !checkPluginConfigured(configPath)) {
      const pluginPath = findPluginPath(import.meta.url)
      console.error("\x1b[33m⚠ Ginny plugin not found in OpenCode config.\x1b[0m")
      console.error("  Session tracking and subagent model selection won\'t work.")
      console.error(`  Fix: ginny setup`)
      console.error("")
    }
  } catch { /* non-fatal */ }
  ...
  // Enable disk auto-discovery when no GINNY_PROFILES env var is set.
  // This lets `ginny profile add` work without restarting the server.
  if (!profiles) {
